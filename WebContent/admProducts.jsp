<%@ page language="java" contentType="text/html; charset=UTF-8"
	import="java.util.ArrayList,java.util.*,com.javaee.bean.*,com.javaee.bean.PageModel"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
		...
		<div class="row">
			...
			<div class="col-lg-12 col-md-12">
				<div class="dashboard-list-box margin-top-0">
					<h4>商品列表</h4>
					<ul>
						<form method="post" action="" name="manageProducts">
							<% 
						...
						<li>
							<div class="list-box-listing">
								...
								<div class="list-box-listing-content">
									<div class="inner">
										...
										<span></span><input title="dishprice" type="text" name="dishprice<%=dishlist.get(i).getDishId()%>" value=<%=dishlist.get(i).getDishPrice()%> style="border-color:#FFFFFF;background:transparent;height:20px;width:100px;padding-top:auto;padding-bottom:auto;margin-top:-23px;margin-left:60px">
										</span> 
										<span>
										<label style="margin-top:-10px;margin-left:20px;margin-bottom:-10px">剩余库存:</label></span>
										<span></span><input title="dishstock" type="text" name="dishstock<%=dishlist.get(i).getDishId()%>" value=<%=dishlist.get(i).getDishStock()%> style="border-color:#FFFFFF;background:transparent;height:20px;width:400px;padding-top:auto;padding-bottom:auto;margin-top:-23px;margin-left:75px">
										</span>
										<span>
										<label style="margin-top:-10px;margin-left:20px;margin-bottom:-10px">评分:</label></span>
										<span></span><input title="dishmarks" type="text" name="dishmarks<%=dishlist.get(i).getDishId()%>" value=<%=dishlist.get(i).getDishMarks()%> style="border-color:#FFFFFF;background:transparent;height:20px;width:400px;padding-top:auto;padding-bottom:auto;margin-top:-23px;margin-left:75px">
										</span>
									</div>
								</div>
							</div>
							<div class="buttons-to-right">
								...
								<input type="submit" id = <%=dishlist.get(i).getDishId()%>  value="编辑" name="UpdateProduct" onclick="checkaction(1,this);" class="button gray" style="margin-right:-10px;width:50px;height:30px;font-size:14px"></a>
								<a href="#" class="button gray"><i class="sl sl-icon-close"></i> 
								<input type="submit" id = <%=dishlist.get(i).getDishId()%> value="删除" name="DeleteProduct" onclick="checkaction(0,this);" class="button gray" style="margin-right:-10px;width:50px;height:30px;font-size:14px"></a>
							</div>
							<script>
							   function checkaction(v,now){
								   if(v==1){
									   document.manageProducts.action="admmanage?action=updateproducts&DishId="+now.id;
								   }
								   else{
									   document.manageProducts.action="admmanage?action=deleteproducts&DishId="+now.id;
								   }
							   }
							</script>
							</li>
						...
						<div align="left" style="margin-left:30px;margin-top:20px;margin-bottom:20px">
												...
												<div align="right" style="margin-right:70px;margin-top:-50px;font-size:18px;">
													<a name="btnTopPage" id="btnTopPage" href="admmanage?action=pagelistproduct&pageNo=<%=pageModel.getPageNo() %>" title="首页">|&lt;&lt;
													</a>&nbsp; &nbsp; <a name="btnPreviousPage" id="btnPreviousPage"
														href="admmanage?action=pagelistproduct&pageNo=<%=pageModel.getPrev() %>" title="上页"> &lt; </a>&nbsp;&nbsp;  <a name="btnNextPage"
														id="btnNextPage" href="admmanage?action=pagelistproduct&pageNo=<%=pageModel.getNext() %>" title="下页"> &gt; </a>&nbsp; &nbsp; <a
														name="btnBottomPage" id="btnBottomPage" href="admmanage?action=pagelistproduct&pageNo=<%=pageModel.getBottom() %>" title="尾页">
														&gt;&gt;|</a>
