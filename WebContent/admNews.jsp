		...
		<div class="row">
			...
			<div class="col-lg-12 col-md-12">
				<div class="dashboard-list-box margin-top-0">
					<h4>新闻列表</h4>
					<ul>
						<form method="post" action="" name="manageNews">
							<% 
							 request.setCharacterEncoding("utf-8"); 
						     response.setCharacterEncoding("utf-8"); 
						     PageModel<News> pageModel= (PageModel<News>) request.getAttribute("pageModel");
						     ArrayList<News> newslist = pageModel.getList();
							for (int i = 0; i < newslist.size(); i++) {%>
						<li>
							<div class="list-box-listing">
								<div class="list-box-listing-img"><a href="#"><img src="<%=newslist.get(i).getNImage()%>" alt=""></a></div>
								<div class="list-box-listing-content">
									<div class="inner">
										<h3><a href="#">
										<input title="ntitle" type="text" name="ntitle<%=newslist.get(i).getNId()%>" value=<%=newslist.get(i).getNTitle()%> style="border-color:#FFFFFF;background:transparent;height:20px;width:400px;padding-top:auto;padding-bottom:auto;margin-top:-20px;color:#F91942">
										</a></h3>
										<span>
										<label style="margin-top:-10px;margin-left:20px;margin-bottom:-10px">作者:</label></span>
										<span></span><input title="authorname" type="text" name="authorname<%=newslist.get(i).getNId()%>" value=<%=newslist.get(i).getNAuthorName()%> style="border-color:#FFFFFF;background:transparent;height:20px;width:100px;padding-top:auto;padding-bottom:auto;margin-top:-23px;margin-left:60px">
										</span> 
										<span>
										<label style="margin-top:-10px;margin-left:20px;margin-bottom:-10px">作者邮箱:</label></span>
										<span></span><input title="authormail" type="text" name="authormail<%=newslist.get(i).getNId()%>" value=<%=newslist.get(i).getNAuthorMail()%> style="border-color:#FFFFFF;background:transparent;height:20px;width:400px;padding-top:auto;padding-bottom:auto;margin-top:-23px;margin-left:75px">
										</span>
									</div>
								</div>
							</div>
							<div class="buttons-to-right">
								...
								<input type="submit" id = <%=newslist.get(i).getNId()%>  value="编辑" name="UpdateNews" onclick="checkaction(1,this);" class="button gray" style="margin-right:-10px;width:50px;height:30px;font-size:14px"></a>
								<a href="#" class="button gray"><i class="sl sl-icon-close"></i> 
								<input type="submit" id = <%=newslist.get(i).getNId()%> value="删除" name="DeleteNews" onclick="checkaction(0,this);" class="button gray" style="margin-right:-10px;width:50px;height:30px;font-size:14px"></a>
							</div>
							<script>
							   function checkaction(v,now){
								   if(v==1){
									   document.manageNews.action="admmanage?action=updatenews&NId="+now.id;
								   }
								   else{
									   document.manageNews.action="admmanage?action=deletenews&NId="+now.id;
								   }
							   }
							</script>
							</li>
						...
						<div align="left" style="margin-left:30px;margin-top:20px;margin-bottom:20px">
												...
												<div align="right" style="margin-right:70px;margin-top:-50px;font-size:18px;">
													<a name="btnTopPage" id="btnTopPage" href="admmanage?action=pagelistnews&pageNo=<%=pageModel.getPageNo() %>" title="首页">|&lt;&lt;
													</a>&nbsp; &nbsp; <a name="btnPreviousPage" id="btnPreviousPage"
														href="admmanage?action=pagelistnews&pageNo=<%=pageModel.getPrev() %>" title="上页"> &lt; </a>&nbsp;&nbsp;  <a name="btnNextPage"
														id="btnNextPage" href="admmanage?action=pagelistnews&pageNo=<%=pageModel.getNext() %>" title="下页"> &gt; </a>&nbsp; &nbsp; <a
														name="btnBottomPage" id="btnBottomPage" href="admmanage?action=pagelistnews&pageNo=<%=pageModel.getBottom() %>" title="尾页">
														&gt;&gt;|</a>
