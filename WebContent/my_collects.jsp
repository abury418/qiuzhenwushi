					...
					<%for (int i = 0; i < collects.size(); i++) { %>
						<li>
							<div class="list-box-listing">
								<div class="list-box-listing-img"><a href="#"><img src="<%=collects.get(i).getSImage() %>" alt=""></a></div>
								<div class="list-box-listing-content">
									<div class="inner">
										<h3><a href="#"><%=collects.get(i).getSName() %></a></h3>
										<span><%=collects.get(i).getSLei() %>, (<%=collects.get(i).getCommentSize() %>笔记)</span>
										<br><div class="rating-counter">收藏时间: <%=collects.get(i).getCOTime() %></div>
									</div>
								</div>
							</div>
							<div class="buttons-to-right">
								<a href="#" class="button gray"><i class="sl sl-icon-note"></i> 添加笔记</a>
								<a href="./showctrl?action=deletecollect&SId=<%=collects.get(i).getSId() %>&UserId=<%=user.getUserId() %>" class="button gray"><i class="sl sl-icon-close"></i> 删除</a>
							</div>
						</li>
						<%} %>
